...
namespace BottleRocket
{
    public class Item
    {
        ...
        //the last byte of the last entry is 1C0F
        public const int END_OFFSET = 0x1C10;
        //There are 128 items in the table, though quite a few of them are dummied out
        public const int NUMBER_OF_ENTRIES = 128;
        ...
        public byte[] GenerateTableEntry()
        {
            ...
            var namePointerBytes = textOffsetTemp.ToString("X4");

            //5A

            var digit2 = Convert.ToByte(
                HexHelpers.InterpretBoolsAsBinary(new[]
                {
                    //3
                    Permanent,     //0
                    Edible,        //0
...
Normal items are just weapons with zero strength, which makes them non-equippable.");
        ...
        public static Item ParseHex(byte[] input)
        {
            ...
            var namePointerTemp =
                HexHelpers.HexStringToInt(input[1].ToString("X2") +
                                          input[0].ToString(
                                              "X2")); //for example, 4A80. we can swap it here so it's 804A
            result.NameTextOffset =
                (namePointerTemp - 0x7FF0)
                .ToString("X4"); //subtract the magic NES number to get the proper ROM offset: 5A

            var itemProperties =
                HexHelpers.IntToBinaryString(input[2]); //get the one with all of the boolean values in it
            itemProperties =
                HexHelpers.Pad(itemProperties, 8); //pad the thing with zeroes so all of the properties can be set

            var strengthAndTypeTemp = HexHelpers.IntToBinaryString(input[3], 8);
            var strengthAndTypeTemp2 =
                HexHelpers.Reverse(
                    strengthAndTypeTemp); //make the string of 1s and 0s not be in reverse order so we can work with it
            ...
            var priceTemp =
                input[7].ToString("X2") +
                input[6].ToString(
                    "X2"); //the price is stored swapped, with no extra math applied to it! We can just swap the numbers like so.
        ...
        {
            //return true for 1 and false for 0. Thanks for the simplification, ReSharper
            return input.Substring(position, 1) == "1"; 
        }
